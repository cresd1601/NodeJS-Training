  ...
  describe('status 200', () => {
    it('Get user favorite items', async () => {
      ...
      const response = await request(app)
        .get(`api/v1/users/:userId/favorite-items`)
        .set(
          'Authorization',
          generateJWT(
...
describe('POST /favorite-items', () => {
  describe('status 200', () => {
    it('Add user favorite item', async () => {
      ...
      const response = await request(app)
        .post(`api/v1/users/:userId/favorite-items`)
        .set(
          'Authorization',
          generateJWT(
            { secretOrKey: process.env.JWT_AUTHENTICATE_SECRET as string },
...
describe('DELETE /favorite-items/:favoriteItemId', () => {
  describe('status 200', () => {
    it('Delete user favorite item', async () => {
      ...
      const response = await request(app)
        .delete(`api/v1/users/:userId/favorite-items/${favoriteItemId}`)
        .set(
          'Authorization',
          generateJWT(
            { secretOrKey: process.env.JWT_AUTHENTICATE_SECRET as string },
